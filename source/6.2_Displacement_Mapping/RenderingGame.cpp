...
namespace Rendering
{
	...
	void RenderingGame::Initialize()
	{
		...
		mDisplacementMappingDemo = make_shared<DisplacementMappingDemo>(*this, camera);
		mComponents.push_back(mDisplacementMappingDemo);
		...
		auto helpTextImGuiRenderBlock = make_shared<ImGuiComponent::RenderBlock>([this]()
		{
			...
			}
			{
				...
				animationEnabledLabel << "Toggle Displacement Map Mode (Space): " << (mDisplacementMappingDemo->RealDisplacementMapEnabled() ? "Real" : "Default");
				ImGui::Text(animationEnabledLabel.str().c_str());
			}
			{
				stringstream ambientLightIntensityLabel;
				ambientLightIntensityLabel << setprecision(2) << "Ambient Light Intensity (+PgUp/-PgDown): " << mDisplacementMappingDemo->AmbientLightIntensity();
				ImGui::Text(ambientLightIntensityLabel.str().c_str());
			}
			{
				stringstream directionalLightIntensityLabel;
				directionalLightIntensityLabel << setprecision(2) << "Directional Light Intensity (+Home/-End): " << mDisplacementMappingDemo->DirectionalLightIntensity();
				ImGui::Text(directionalLightIntensityLabel.str().c_str());
			}
			{
				stringstream displacementScaleLabel;
				displacementScaleLabel << setprecision(2) << "Displacement Scale (+Insert/-Delete): " << mDisplacementMappingDemo->DisplacementScale();
		...
		mAmbientLightIntensity = mDisplacementMappingDemo->AmbientLightIntensity();
		mDirectionalLightIntensity = mDisplacementMappingDemo->DirectionalLightIntensity();
	}
	...
	void RenderingGame::Shutdown()
	{
		...
		mDisplacementMappingDemo = nullptr;
	...
	void RenderingGame::UpdateAmbientLightIntensity(const GameTime& gameTime)
	{
		if (mKeyboard->IsKeyDown(Keys::PageUp) && mAmbientLightIntensity < 1.0f)
		{
			...
			mDisplacementMappingDemo->SetAmbientLightIntensity(mAmbientLightIntensity);
		}
	...
	void RenderingGame::UpdateDirectionalLight(const GameTime& gameTime)
	{
		...
		if (mKeyboard->IsKeyDown(Keys::Home) && mDirectionalLightIntensity < 1.0f)
		{
			...
			mDisplacementMappingDemo->SetDirectionalLightIntensity(mDirectionalLightIntensity);
		}
		...
		{
			mDisplacementMappingDemo->RotateDirectionalLight(rotationAmount);
		}
	}

	void RenderingGame::UpdateDisplacement(const Library::GameTime & gameTime)
	{
		if (mKeyboard->WasKeyPressedThisFrame(Keys::Space))
		{
			mDisplacementMappingDemo->ToggleRealDisplacementMap();
		}

		if (mKeyboard->IsKeyDown(Keys::Insert))
		{
			mDisplacementMappingDemo->SetDisplacementScale(mDisplacementMappingDemo->DisplacementScale() + gameTime.ElapsedGameTimeSeconds().count());
		}
		else if (mKeyboard->IsKeyDown(Keys::Delete))
		{
			mDisplacementMappingDemo->SetDisplacementScale(mDisplacementMappingDemo->DisplacementScale() - gameTime.ElapsedGameTimeSeconds().count());
		}
	}
