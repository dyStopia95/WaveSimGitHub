...
namespace Library
{
	...
	class VertexXYIndex : public VertexDeclaration<VertexXYIndex>
	{
	private:
		inline static const D3D11_INPUT_ELEMENT_DESC _InputElements[]
		{
			{ "XYPOS", 0, DXGI_FORMAT_R32G32_FLOAT, 0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },
			{ "INDEX", 0, DXGI_FORMAT_R32_UINT, 0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },
		};

	public:
		VertexXYIndex() = default;
		VertexXYIndex(const DirectX::XMFLOAT2& xypos, size_t index) :
			XYPos{ xypos }, Index{ index } { };

		DirectX::XMFLOAT2 XYPos;
		size_t Index;

		inline static const gsl::span<const D3D11_INPUT_ELEMENT_DESC> InputElements{ _InputElements };

		//static void CreateVertexBuffer(gsl::not_null<ID3D11Device*> device, const Library::Mesh& mesh, gsl::not_null<ID3D11Buffer**> vertexBuffer);
		static void CreateVertexBuffer(gsl::not_null<ID3D11Device*> device, const gsl::span<const VertexXYIndex>& vertices, gsl::not_null<ID3D11Buffer**> vertexBuffer)
		{
			VertexDeclaration::CreateVertexBuffer(device, vertices, vertexBuffer);
		}
	};
