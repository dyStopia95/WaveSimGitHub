...
namespace Rendering
{
	...
	void RenderingGame::Initialize()
	{
		...
		mTransparencyDemo = make_shared<TransparencyDemo>(*this, camera);
		mComponents.push_back(mTransparencyDemo);
		...
		auto helpTextImGuiRenderBlock = make_shared<ImGuiComponent::RenderBlock>([this]()
		{
			...
			}
			{
				...
				ambientLightIntensityLabel << setprecision(2) << "Ambient Light Intensity (+PgUp/-PgDown): " << mTransparencyDemo->AmbientLightIntensity();
				ImGui::Text(ambientLightIntensityLabel.str().c_str());
			}
			{
				stringstream directionalLightIntensityLabel;
				directionalLightIntensityLabel << setprecision(2) << "Directional Light Intensity (+Home/-End): " << mTransparencyDemo->DirectionalLightIntensity();
				ImGui::Text(directionalLightIntensityLabel.str().c_str());
			}
			{
				stringstream specularIntensityLabel;
				specularIntensityLabel << setprecision(2) << "Specular Intensity (+Insert/-Delete): " << mTransparencyDemo->SpecularIntensity();
				ImGui::Text(specularIntensityLabel.str().c_str());
			}
			{
				stringstream specularPowerLabel;
				specularPowerLabel << "Specular Power (+O/-P): " << mTransparencyDemo->SpecularPower();
				ImGui::Text(specularPowerLabel.str().c_str());
			}
			{
				stringstream fogStartLabel;
				fogStartLabel << "Fog Start (+K/-L): " << mTransparencyDemo->FogStart();
				ImGui::Text(fogStartLabel.str().c_str());
			}
			{
				stringstream fogRangeLabel;
				fogRangeLabel << "Fog Range (+N/-M): " << mTransparencyDemo->FogRange();
		...
		mAmbientLightIntensity = mTransparencyDemo->AmbientLightIntensity();
		mDirectionalLightIntensity = mTransparencyDemo->DirectionalLightIntensity();
		mSpecularIntensity = mTransparencyDemo->SpecularIntensity();
		mSpecularPower = mTransparencyDemo->SpecularPower();
		mFogStart = mTransparencyDemo->FogStart();
		mFogRange = mTransparencyDemo->FogRange();
	}
	...
	void RenderingGame::UpdateAmbientLightIntensity(const GameTime& gameTime)
	{
		if (mKeyboard->IsKeyDown(Keys::PageUp) && mAmbientLightIntensity < 1.0f)
		{
			...
			mTransparencyDemo->SetAmbientLightIntensity(mAmbientLightIntensity);
		}
	...
	void RenderingGame::UpdateDirectionalLight(const GameTime& gameTime)
	{
		...
		if (mKeyboard->IsKeyDown(Keys::Home) && mDirectionalLightIntensity < 1.0f)
		{
			...
			mTransparencyDemo->SetDirectionalLightIntensity(mDirectionalLightIntensity);
		}
		...
		{
			mTransparencyDemo->RotateDirectionalLight(rotationAmount);
		}
	}
	...
	void RenderingGame::UpdateSpecularLight(const Library::GameTime & gameTime)
	{
		...
		if (mKeyboard->IsKeyDown(Keys::Insert) && mSpecularIntensity < 1.0f)
		{
			...
			mTransparencyDemo->SetSpecularIntensity(mSpecularIntensity);
		}
		...
		if (mKeyboard->IsKeyDown(Keys::O) && mSpecularPower < numeric_limits<uint8_t>::max())
		{
			...
			mTransparencyDemo->SetSpecularPower(mSpecularPower);
		}
	...
	void RenderingGame::UpdateFog(const GameTime& gameTime)
	{
		...
		if (mKeyboard->IsKeyDown(Keys::K))
		{
			...
			mTransparencyDemo->SetFogStart(mFogStart);
		}
		...
		if (mKeyboard->IsKeyDown(Keys::N))
		{
			...
			mTransparencyDemo->SetFogRange(mFogRange);
		}
